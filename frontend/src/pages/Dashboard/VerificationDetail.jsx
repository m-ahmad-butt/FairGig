...
function isNumericMatch(observed, expected, toleranceRatio = 0.1) {
  ...
  return (
    Math.abs(observedNumber - expectedNumber) / denominator <= toleranceRatio
  );
}
...
export default function VerificationDetailPage() {
  ...
  const loadAiAnalysis = async ({ evidenceData, sessionData, earningData }) => {
    ...
    try {
      ...
      const agentResult = await earningsService.verifyScreenshotWithAI(
        workerId,
        sessionId,
      );

      const detectedGross = toNumber(
        agentResult?.data_from_picture?.gross_amount,
      );
      const detectedDeductions = toNumber(
        agentResult?.data_from_picture?.platform_deduction,
      );
      const detectedNet = toNumber(agentResult?.data_from_picture?.net_amount);
      const detectedPlatform =
        agentResult?.data_from_picture?.platform_name ||
        sessionData?.platform ||
        "Unknown";
      const loggedGross = toNumber(
        agentResult?.actual_data?.gross_amount ?? earningData?.gross_earned,
      );
      const loggedDeductions = toNumber(
        agentResult?.actual_data?.platform_deduction ??
          earningData?.platform_deductions,
      );
      const loggedNet = toNumber(
        agentResult?.actual_data?.net_amount ?? earningData?.net_received,
      );
      const loggedPlatform =
        agentResult?.actual_data?.platform_name ||
        sessionData?.platform ||
        "Unknown";
      const loggedDate =
        sessionData?.session_date || new Date().toISOString().split("T")[0];
      const anomalyTypes = Array.isArray(agentResult?.anomaly_types)
        ? agentResult.anomaly_types
        : [];
      const anomalyDetected = Boolean(agentResult?.anomaly_detected);
      const confidenceScore = toNumber(agentResult?.confidence_score);
      const confidenceThreshold = toNumber(
        agentResult?.anomaly_confidence_threshold || 80,
      );
      ...
      const mappedAnalysis = {
        ...
        discrepancies: [
          ...
          },
          {
            ...
            match:
              normalizePlatform(detectedPlatform) ===
              normalizePlatform(loggedPlatform),
          },
          {
            field: "date",
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-6">
        <div className="mb-6">
          <div className="flex items-center gap-2 text-sm text-gray-500 mb-2">
            <Link to="/verifier/dashboard" className="hover:text-gray-900">
              Review Queue
            </Link>
            <span>›</span>
          ...
          <div className="lg:col-span-3">
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
              ...
              <div className="text-sm text-gray-500 mb-3">
                USER UPLOAD:{" "}
                {evidence?.image_url?.split("/").pop() || "EVIDENCE.PNG"}
              </div>
                  ...
                  <div className="space-y-4">
                    {aiLoading ? (
                      ...
                      <>
                        <div>
                          ...
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              ...
                              <span
                                className={
                                  aiAnalysis.extraction_summary.detected_gross
                                    ? "text-gray-900"
                            ...
                            <div className="flex justify-between">
                              ...
                              >
                                {aiAnalysis.extraction_summary
                                  .detected_deductions
                                  ? `-${formatCurrency(aiAnalysis.extraction_summary.detected_deductions)}`
                                  : "— not detected"}
                              </span>
                            </div>
                            <div className="flex justify-between">
                              ...
                              <span
                                className={
                                  aiAnalysis.extraction_summary.detected_net
                            ...
                            <div className="flex justify-between">
                              ...
                              >
                                {aiAnalysis.extraction_summary
                                  .detected_platform || "— not detected"}
                              </span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-gray-500">Date</span>
                              <span
                                className={
                                  aiAnalysis.extraction_summary.detected_date
                                    ? "text-gray-900"
                                    : "text-gray-400 italic"
                                }
                              >
                          ...
                          <div className="space-y-2">
                            {aiAnalysis.discrepancies.map((item, i) => {
                              ...
                              return (
                                <div
                                  key={i}
                                  className={`flex items-center justify-between text-sm py-2 border-b border-gray-100 last:border-0 ${isMismatch ? "bg-red-50 -mx-2 px-2 rounded" : ""}`}
                                >
                                  ...
                                  <div className="flex items-center gap-2 text-xs">
                                    ...
                                    <span
                                      className={`w-20 ${hasDetected ? "text-gray-900" : "text-gray-400 italic"}`}
                                    >
                                      {detectedDisplay}
                                    </span>
                        ...
                        <div
                          className={`border-2 rounded-xl p-4 ${getVerdictStyle(aiAnalysis.verdict)}`}
                        >
                          <div className="flex items-center justify-between mb-2">
                            <span
                              className={`font-medium ${
                                aiAnalysis.verdict === "likely_valid"
                                  ? "text-green-700"
                                  : aiAnalysis.verdict === "discrepancy_found"
                                    ? "text-red-700"
                                    : "text-gray-600"
                              }`}
                            >
                        ...
                        <p className="text-xs text-gray-400 italic">
                          AI analysis is advisory only. Final verification
                          decision rests with the reviewer.
                        </p>

                        {aiAnalysis.cached && (
                          <p className="text-xs text-gray-400">
                            Showing cached agent result
                            {aiAnalysis.evaluated_at
                              ? ` from ${new Date(aiAnalysis.evaluated_at).toLocaleString("en-PK")}`
                              : ""}
                            .
                          </p>
                        )}
              ...
              <div className="px-4 pb-4">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  ...
                  <span className="text-gray-400">
                    (REQUIRED FOR FLAG / UNVERIFIABLE)
                  </span>
                </label>
                <textarea
                  value={reviewerNotes}
