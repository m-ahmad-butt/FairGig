...
function getActivityStatus(session) {
  ...
  if (isVerified || evidenceStatus === "verified") {
    return {
      label: "VERIFIED",
  ...
  if (evidenceStatus === "unverifiable") {
    return {
      label: "UNVERIFIABLE",
...
export default function WorkerDashboardPage() {
  ...
  useEffect(() => {
    ...
    fetchWorkerNotifications(workerId);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const handleNotification = (payload) => {
        ...
        if (payload.user_id && payload.user_id !== workerId) {
          return;
        }
  ...
  const fetchWorkerNotifications = async (targetWorkerId = workerId) => {
    if (!targetWorkerId) {
      return;
    }
    ...
    try {
      const fetched =
        await earningsService.getWorkerNotifications(targetWorkerId);
      setNotifications(Array.isArray(fetched) ? fetched : []);
    } catch (error) {
      console.error("Failed to fetch worker notifications:", error);
    }
  };
  ...
  const markUnreadNotificationsAsRead = async () => {
    ...
    setNotifications((previous) =>
      previous.map((item) =>
        unreadIds.has(item.id) ? { ...item, read: true } : item,
      ),
    );

    await Promise.allSettled(
      unread.map((item) =>
        earningsService.markNotificationAsRead(item.id, workerId),
      ),
    );
  };
  ...
  const loadData = async () => {
    try {
      ...
      await fetchWorkerNotifications(workerId);
      ...
      const merged = rawSessions
        .map((session) => {
          const earning = allEarnings.find(
        ...
        .sort(
          (left, right) =>
            new Date(right.session_date) - new Date(left.session_date),
        );
      ...
      const verifiedEarnings = merged
        ...
        .reduce(
          (sum, session) => sum + Number(session.earning?.net_received || 0),
          0,
        );
      ...
      const detectedAnomalies = anomalyData.anomalies.filter(
        (a) => a.anomaly_detected,
      );
      const anomalyCount =
  ...
  const trendDrawing = useMemo(
    () => buildTrendPath(weeklyTrend),
    [weeklyTrend],
  );
  const hasTrendData = useMemo(
    () => weeklyTrend.some((point) => point.value > 0),
    [weeklyTrend],
  );
      ...
      <div className="mx-auto max-w-6xl px-4 pt-5 sm:px-6 lg:px-8">
        <div className="mb-4 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-600">
              Your weekly performance and recent activity.
            </p>
          </div>
          ...
          <div className="relative">
            <button
              ...
              className="relative inline-flex h-11 w-11 items-center justify-center rounded-xl border border-zinc-200 bg-white text-zinc-700 shadow-sm transition-colors hover:bg-zinc-50"
            ...
            {notificationDropdownOpen && (
              <div className="absolute right-0 z-30 mt-2 w-[360px] max-w-[90vw] overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-lg">
                    ...
                    orderedNotifications.map((notification) => {
                      const canResolve =
                        (notification.type === "evidence_flagged" ||
                          notification.type === "evidence_unverifiable") &&
                      ...
                      return (
                        <div
                          ...
                          className={`border-b border-zinc-100 px-4 py-3 last:border-b-0 ${
                            notification.read ? "bg-white" : "bg-zinc-50"
                          }`}
                        >
                          <p className="text-sm font-semibold text-zinc-900">
                            ...
                            {canResolve && (
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  navigate(
                                    `/worker/resolve/${notification.data.session_id}`,
                                  );
                                }}
                                className="rounded-md bg-zinc-900 px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-zinc-800"
                              >
                                Resolve
                              </button>
                            )}
        ...
        <div className="grid gap-4 lg:grid-cols-[2fr_1fr]">
          <div className="rounded-xl border border-[#2f3f5a] bg-gradient-to-br from-[#1b273c] via-[#2d3e5a] to-[#374559] p-5 text-white shadow-lg">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-200/90">
              Verified Earnings
            </p>
            <div className="mt-2 flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              <div>
                <p className="text-4xl font-extrabold tracking-tight sm:text-5xl">
                  {formatCurrency(stats.verifiedEarnings)}
                </p>
              ...
              <div className="h-40 w-full max-w-[340px] rounded-lg border border-white/10 bg-white/5 p-2">
                {hasTrendData ? (
                  ...
                  <div className="flex h-full items-center justify-center text-xs text-zinc-200/80">
                  ...
                  {topAnomaly ? (
                    topAnomaly.explanation ? (
                      ...
                      <p className="mt-1 text-sm">
                        Unusual deduction detected on{" "}
                        {formatDateLabel(topAnomaly.session_date)} -{" "}
                        {Math.round(topAnomaly.deductionPercent)}% vs your
                        usual.
                      </p>
                    )
                  ) : (
                    <p className="mt-1 text-sm">
                      No high-risk deductions detected in recent sessions.
                    </p>
                  )}
                </div>
              </div>
                ...
                <tbody>
                  {recentSessions.map((session, index) => {
                    ...
                    const deduction = Number(
                      session.earning?.platform_deductions || 0,
                    );
                    const deductionPercent =
                      gross > 0 ? (deduction / gross) * 100 : 0;
                    ...
                    return (
                      ...
                      >
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-zinc-500">
                                {formatPlatformLabel(session.platform)}
                              </p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          {formatDuration(session.hours_worked)}
                        </td>
